    ...
    internal class LicVerifier : IVerifier_Signer, IVerifier_ApplicationCode, IVerifier_VerifyLoad
    {
        ...
        SignedLicense IVerifier_VerifyLoad.LoadAndVerify(string licenseString)
        {
            ...
            if (!SerialNumber.IsApplicationCodeValid(license.SerialNumber, myApplicationCode))
                license.VerifyExceptions.Add(new ProductMismatchException($"Application Code '{myApplicationCode}' is not valid for the license.", myApplicationCode));
            // verify hardware identifier
            if (!HardwareIdentifier.IsValidForCurrentComputer(license.HardwareIdentifier))
                license.VerifyExceptions.Add(new HardwareIdentifierException($"License has been activated for another computer."));
            // verify expiration date
            if (license.ExpirationDate < DateTime.UtcNow)
                license.VerifyExceptions.Add(new LicenseExpiredException($"License has been expired since '{license.ExpirationDate}'.", license.ExpirationDate));
